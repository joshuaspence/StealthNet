 ...
 * 
 * <p> A nonce is generated by a pseudo-random random generator. The nonce is
 * used to prevent replay attacks, because once the packet is read at the
 * receiving end of the communications, the nonce is "consumed" and cannot be
 * used again for future communications. The {@link DecryptedPacket} class
 * <em>will</em> allow packets to be created without a nonce (if a null
 * {@link NonceGenerator} instance is passed to the function). A higher layer
 * should check whether or not this should be allowed.
 * 
 * @author Matt Barrie
...
public class DecryptedPacket {
	 ...
	 * base-64.
	 * 
	 * <p> If the {@link Client} wishes to cancel an in-progress transaction
	 * (possibly because they have insufficient funds in the {@link Bank}, then
	 * the {@link Client} client should send a "null" payment.
	 * 
	 * <p> A {@link Client} can make a "payment" to the {@link Bank}, in order
	 * to refund unspent {@link CryptoCredit}s from the {@link Client} 's
	 * {@link CryptoCreditHashChain} to the user's account.
	 * 
	 * <p> The {@link Server} can also send {@link CryptoCredit}s to the
	 * {@link Client}, to effectively withdraw funds from the user's account.
	 */
	 ...
	 * 
	 * <p> The hash sent from the {@link Client} to the {@link Bank} is encoded
	 * in base-64. Similarly, the signature sent from the {@link Bank} to the
	 ...
	 * is valid.
	 * 
	 * <p> The peer sends the data in the form <code>userID;credits;hash</code>.
	 * The hash is to be encoded in base-64. The {@link Bank} sends data in the
	 * form <code>[true|false]</code>.
	 */
